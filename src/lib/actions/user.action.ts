...
import { assignBadegs } from '../utils'
...
export async function getUserInfo(params: GetUserByIdParams) {
  try {
    ...
    const badgeCounts = assignBadegs({ criteria })

    return { totalAnswers, totalQuestions, user, badgeCounts, reputation: user.reputation }
  } catch (error) {
    console.log(error)
    throw error
  }
...
export async function getUserAnswers(params: GetUserStatsParams) {
  try {
    ...
    return { totalQuestions: totalAnswers, answers: userAnswers, isNext }
  } catch (error) {
    console.log(error)
    throw error
  }
